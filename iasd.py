...
class Person:
    ...
    # trait vector for species matching
    trait_vector: Optional[List[float]] = None
...
def species_compatible(sp_parent: Species, sp_child: Species) -> bool:
    # Domain-specific; here we assume same species code is required
...
class IASDProcedures:
    ...
    def ancestry_edges(self, person_id: str, k: int) -> List[Tuple[Person, Person]]:
        ...
        # For each ancestor, include edges to children that are on a path to the target
    ...
    def ancestry_coverage(self, person_id: str, k: int) -> float:
        ancestors = self.ancestry_tree(person_id, k)
        if not ancestors:
            return 1.0  # degenerate: no known ancestors
    ...
    def ancestry_violations(self, person_id: str, k: int, epsilon_years: float = 0.5) -> float:
        ...
        for par, ch in edges:
            ...
            if sp_par is None or sp_ch is None:
                # If species unknown, skip species-based constraints
                continue
    ...
    def species_anomaly(self, person_id: str, sigma: float) -> float:
        ...
        if p.claimed_species_id not in self.g.species:
            return 0.0  # unknown claimed species
        ...
        best_species, sim_best = max(sims, key=lambda x: x[1])
...
class IASDConfig:
    ...
    # thresholds etc. can be added here
...
class IASDEngine:
    ...
    def crca_score(self, person_id: str) -> float:
        """
        Simple coherence score based on record footprint:
        - More systems + more records + higher validity => lower anomaly.
        - Here we just invert a crude density measure as a placeholder.
        """
        summaries = self.proc.record_footprint(person_id)
        if not summaries:
            return 1.0  # no records at all: highly anomalous

        # crude density: sum of log(1 + n_records) weighted by avg_validity
        density = 0.0
        for s in summaries:
            n = s["n_records"]
        ...
        # map density to anomaly in (0,1] via 1 / (1 + density)
    ...
    def compute_features(self, person_id: str) -> Dict:
        ...
        # convert coverage to anomaly: low coverage => high anomaly
    ...
    def explain(self, person_id: str) -> Dict:
        """
        Return both the score and the decomposed feature contributions.
        """
...
if __name__ == "__main__":
    # Minimal demo wiring; replace with your own data loading.
    g = Graph()
    ...
    # Imposter with no ancestry and weird traits
    p2 = Person(
        id="P2",
        external_id="X-999",
